define(function(){
	function createAnswerView(answerInfo){
        ...
        console.log("focus: " + answerInfo.focus + " showEntry: " + answerInfo.showEntry + " reScan: " + answerInfo.reScan);
        if (answerInfo.focus == "smarthome") {
            if (answerInfo.reScan && answerInfo.reScan == 1) {
                answer_text.style.paddingBottom = 0 * scale + "px";
                var linkDiv = document.createElement("a");
                linkDiv.innerText = "重新扫描";
                linkDiv.className = "css_dialog_mode_smart_home_link";
                linkDiv.onclick = function() {
                    exec('SmartHomeBusinessHandler', "scanSmartDevices", []);
                    console.log("onclick link to rescan");
                };
                dialog_answer_text.appendChild(linkDiv);
                console.log("add link to rescan");
            } else if (answerInfo.showEntry) {
                var entryName = answerInfo.showName;
                if (entryName) {
                    answer_text.style.paddingBottom = 0 * scale + "px";
                    var linkDiv = document.createElement("a");
                    linkDiv.innerText = entryName + " >";
                    linkDiv.className = "css_dialog_mode_smart_home_link";
                    linkDiv.onclick = function() {
                        exec('SmartHomeBusinessHandler', "handleAppEntry", [answerInfo.showEntry]);
                        console.log("onclick link to downloadPlugin");

                    };
                    dialog_answer_text.appendChild(linkDiv);
                    console.log("add link to enter plugin");
                }
            }
        }
