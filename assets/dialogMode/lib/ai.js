...
var pullLoadMoreHelper = {
    init: function (sohunewsSlipJs, slip_up_arrow, slip_up_text, upDownDiv) {
        this.sohunewsSlipJs = sohunewsSlipJs;
        this.slip_up_arrow = slip_up_text;
        this.slip_up_text = slip_up_text;
        this.upDownDiv = upDownDiv;
    },
//    var sohunewsSlipJs;
//    var slip_up_arrow;
//    var slip_up_text;
//    var upDownDiv;
    ...
    stopRefresh: function () {
        ...
        this.sohunewsSlipJs.up_range = 0;
        this.sohunewsSlipJs.refresh();
    ...
    //创建加载更多的底部视图
    createLoadMoreDiv: function (mainDiv) {
    ...
    //开启加载更多视图
    enablePullLoadMore: function (slipDivOutDiv, slipDiv, callBack) {
        var minus = ai.ovb.ios() && ai.ovb.safari() && !ai.ovb.ipad() ? -20 : 40;
        up_ing = false,
        down_ing = false,
        loadb = false;
        slipDivOutDiv.style.height = ai.wh() - minus + "px";
        slipDivOutDiv.style.height = ai.wh() - minus + "px";
        slipDivOutDiv.style.overflow = "hidden";
        function update() {
            console.log(this.xy);
            console.log(this.coreWidth_cut_width);
            if (this.xy < 0 && this.xy < -(this.coreWidth_cut_width + 20) && up_ing == false) {
                up_ing = true;
                down_ing = false;
//                console.log(this.up_range);

                pullLoadMoreHelper.slip_up_arrow.style['webkitTransitionDuration'] = '300ms';
                pullLoadMoreHelper.slip_up_arrow.style['webkitTransform'] = 'rotate(0deg)';
                pullLoadMoreHelper.slip_up_text.innerHTML = "松开加载更多";
                loadb = true;

            } else if (this.xy < 0 && this.xy > -(this.coreWidth_cut_width + 20) && down_ing == false) {
                down_ing = true;
                up_ing = false;
//                console.log(this.up_range);
                pullLoadMoreHelper.slip_up_arrow.style['webkitTransitionDuration'] = '300ms';
                pullLoadMoreHelper.slip_up_arrow.style['webkitTransform'] = 'rotate(-180deg)';
                pullLoadMoreHelper.slip_up_text.innerHTML = "上拉加载更多";
                loadb = false;
            }
        }
        ...
        function loading() {
            if (loadb) {
                loadb = false;
                var that = this;
                pullLoadMoreHelper.slip_up_text.innerHTML = 'Loading...';
                pullLoadMoreHelper.slip_up_arrow.style['webkitTransitionDuration'] = '0ms';
                pullLoadMoreHelper.slip_up_arrow.className += ' loading';
                callBack();//加载新闻
            }
        }

        this.sohunewsSlipJs = slip('px', slipDiv, {
            moveFun: update,
            endFun: loading
        });
    ...
    disablePullLoadMore: function () {
        ...
        this.sohunewsSlipJs.up_range = 0;
        this.sohunewsSlipJs.refresh();
        this.sohunewsSlipJs.moveFun = emptyFunctin;
        this.sohunewsSlipJs.loading = emptyFunctin;
    }
